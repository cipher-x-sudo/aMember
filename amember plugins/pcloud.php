...
class Am_Storage_Pcloud extends Am_Storage
{
    ...
    function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addSecretText('password')
            ->setLabel('Password')
            ->addRule('required');
        $form->addSelect('location')
            ->setLabel('Data Center Location')
            ->loadOptions([
                'us' => 'United States',
                'eu' => 'Europe',
            ]);
    ...
    {
        switch ($this->getConfig('location', 'us')) {
            case 'us':
    ...
    function directAction($request, $response, $invokeArgs)
    {
        ...
        $url = json_encode($this->getEndpoint() . '/getpublinkdownload?' . http_build_query([
                'forcedownload' => 1,
...
CUT;
    ...
    {
        //link is working only for requester IP, so we can not do server side call to getpublinkdownload
...
//        we can not use it because link is bound to server IP and user can not use it in his browser
...
//        return $this->getpublinkdownload($code, $force_download);
    ...
    {
        $req = new Am_HttpRequest($this->getEndpoint() . "/listfolder?" . http_build_query([
    ...
    {
        $req = new Am_HttpRequest($this->getEndpoint() . "/getfilepublink?" . http_build_query([
                'username' => $this->getConfig('username'),
                'password' => $this->getConfig('password'),
                'path' => $path,
    ...
    {
        $req = new Am_HttpRequest($this->getEndpoint() . "/getpublinkdownload?" . http_build_query([
                'code' => $code,
    ...
    {
        $req = new Am_HttpRequest($this->getEndpoint() . "/getfilelink?" . http_build_query([
                'username' => $this->getConfig('username'),
                'password' => $this->getConfig('password'),
                'path' => $path,
                'forcedownload' => $force_download,
    ...
    {
        $req = new Am_HttpRequest($this->getEndpoint() . "/checksumfile?" . http_build_query([
                'username' => $this->getConfig('username'),
                'password' => $this->getConfig('password'),
                'path' => $path,
            ]));
